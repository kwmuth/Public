$modde2
ORG 0000H

ljmp lookup
$include(math16.asm) 

BSEG
mf: dbit 1

DSEG at 30H
x: DS 2
y: DS 2
BCD: DS 3 

CSEG
; Look-up table for 7-seg displays
myLUT:
    DB 0C0H, 0F9H, 0A4H, 0B0H, 099H        ; 0 TO 4
    DB 092H, 082H, 0F8H, 080H, 090H        ; 4 TO 9
    DB 088H, 083H, 0C6H, 0A1H, 086H, 08EH  ; A to F
; Look-up table for 7-seg displays. 
T_7seg:
    DB 40H, 79H, 24H, 30H, 19H
    DB 12H, 02H, 78H, 00H, 10H

mytable:
DW -400 ; 0000D
DW -399 ; 0001D
DW -397 ; 0002D
DW -396 ; 0003D
DW -395 ; 0004D
DW -393 ; 0005D
DW -392 ; 0006D
DW -390 ; 0007D
DW -389 ; 0008D
DW -388 ; 0009D
DW -386 ; 0010D
DW -385 ; 0011D
DW -384 ; 0012D
DW -382 ; 0013D
DW -381 ; 0014D
DW -380 ; 0015D
DW -378 ; 0016D
DW -377 ; 0017D
DW -375 ; 0018D
DW -374 ; 0019D
DW -373 ; 0020D
DW -371 ; 0021D
DW -370 ; 0022D
DW -369 ; 0023D
DW -367 ; 0024D
DW -366 ; 0025D
DW -364 ; 0026D
DW -363 ; 0027D
DW -362 ; 0028D
DW -360 ; 0029D
DW -359 ; 0030D
DW -358 ; 0031D
DW -356 ; 0032D
DW -355 ; 0033D
DW -354 ; 0034D
DW -352 ; 0035D
DW -351 ; 0036D
DW -349 ; 0037D
DW -348 ; 0038D
DW -347 ; 0039D
DW -345 ; 0040D
DW -344 ; 0041D
DW -343 ; 0042D
DW -341 ; 0043D
DW -340 ; 0044D
DW -339 ; 0045D
DW -337 ; 0046D
DW -336 ; 0047D
DW -334 ; 0048D
DW -333 ; 0049D
DW -332 ; 0050D
DW -330 ; 0051D
DW -329 ; 0052D
DW -328 ; 0053D
DW -326 ; 0054D
DW -325 ; 0055D
DW -324 ; 0056D
DW -322 ; 0057D
DW -321 ; 0058D
DW -319 ; 0059D
DW -318 ; 0060D
DW -317 ; 0061D
DW -315 ; 0062D
DW -314 ; 0063D
DW -313 ; 0064D
DW -311 ; 0065D
DW -310 ; 0066D
DW -309 ; 0067D
DW -307 ; 0068D
DW -306 ; 0069D
DW -304 ; 0070D
DW -303 ; 0071D
DW -302 ; 0072D
DW -300 ; 0073D
DW -299 ; 0074D
DW -298 ; 0075D
DW -296 ; 0076D
DW -295 ; 0077D
DW -293 ; 0078D
DW -292 ; 0079D
DW -291 ; 0080D
DW -289 ; 0081D
DW -288 ; 0082D
DW -287 ; 0083D
DW -285 ; 0084D
DW -284 ; 0085D
DW -283 ; 0086D
DW -281 ; 0087D
DW -280 ; 0088D
DW -278 ; 0089D
DW -277 ; 0090D
DW -276 ; 0091D
DW -274 ; 0092D
DW -273 ; 0093D
DW -272 ; 0094D
DW -270 ; 0095D
DW -269 ; 0096D
DW -268 ; 0097D
DW -266 ; 0098D
DW -265 ; 0099D
DW -263 ; 0100D
DW -262 ; 0101D
DW -261 ; 0102D
DW -259 ; 0103D
DW -258 ; 0104D
DW -257 ; 0105D
DW -255 ; 0106D
DW -254 ; 0107D
DW -253 ; 0108D
DW -251 ; 0109D
DW -250 ; 0110D
DW -248 ; 0111D
DW -247 ; 0112D
DW -246 ; 0113D
DW -244 ; 0114D
DW -243 ; 0115D
DW -242 ; 0116D
DW -240 ; 0117D
DW -239 ; 0118D
DW -238 ; 0119D
DW -236 ; 0120D
DW -235 ; 0121D
DW -233 ; 0122D
DW -232 ; 0123D
DW -231 ; 0124D
DW -229 ; 0125D
DW -228 ; 0126D
DW -227 ; 0127D
DW -225 ; 0128D
DW -224 ; 0129D
DW -222 ; 0130D
DW -221 ; 0131D
DW -220 ; 0132D
DW -218 ; 0133D
DW -217 ; 0134D
DW -216 ; 0135D
DW -214 ; 0136D
DW -213 ; 0137D
DW -212 ; 0138D
DW -210 ; 0139D
DW -209 ; 0140D
DW -207 ; 0141D
DW -206 ; 0142D
DW -205 ; 0143D
DW -203 ; 0144D
DW -202 ; 0145D
DW -201 ; 0146D
DW -199 ; 0147D
DW -198 ; 0148D
DW -197 ; 0149D
DW -195 ; 0150D
DW -194 ; 0151D
DW -192 ; 0152D
DW -191 ; 0153D
DW -190 ; 0154D
DW -188 ; 0155D
DW -187 ; 0156D
DW -186 ; 0157D
DW -184 ; 0158D
DW -183 ; 0159D
DW -182 ; 0160D
DW -180 ; 0161D
DW -179 ; 0162D
DW -177 ; 0163D
DW -176 ; 0164D
DW -175 ; 0165D
DW -173 ; 0166D
DW -172 ; 0167D
DW -171 ; 0168D
DW -169 ; 0169D
DW -168 ; 0170D
DW -166 ; 0171D
DW -165 ; 0172D
DW -164 ; 0173D
DW -162 ; 0174D
DW -161 ; 0175D
DW -160 ; 0176D
DW -158 ; 0177D
DW -157 ; 0178D
DW -156 ; 0179D
DW -154 ; 0180D
DW -153 ; 0181D
DW -151 ; 0182D
DW -150 ; 0183D
DW -149 ; 0184D
DW -147 ; 0185D
DW -146 ; 0186D
DW -145 ; 0187D
DW -143 ; 0188D
DW -142 ; 0189D
DW -141 ; 0190D
DW -139 ; 0191D
DW -138 ; 0192D
DW -136 ; 0193D
DW -135 ; 0194D
DW -134 ; 0195D
DW -132 ; 0196D
DW -131 ; 0197D
DW -130 ; 0198D
DW -128 ; 0199D
DW -127 ; 0200D
DW -126 ; 0201D
DW -124 ; 0202D
DW -123 ; 0203D
DW -121 ; 0204D
DW -120 ; 0205D
DW -119 ; 0206D
DW -117 ; 0207D
DW -116 ; 0208D
DW -115 ; 0209D
DW -113 ; 0210D
DW -112 ; 0211D
DW -111 ; 0212D
DW -109 ; 0213D
DW -108 ; 0214D
DW -106 ; 0215D
DW -105 ; 0216D
DW -104 ; 0217D
DW -102 ; 0218D
DW -101 ; 0219D
DW -100 ; 0220D
DW -098 ; 0221D
DW -097 ; 0222D
DW -095 ; 0223D
DW -094 ; 0224D
DW -093 ; 0225D
DW -091 ; 0226D
DW -090 ; 0227D
DW -089 ; 0228D
DW -087 ; 0229D
DW -086 ; 0230D
DW -085 ; 0231D
DW -083 ; 0232D
DW -082 ; 0233D
DW -080 ; 0234D
DW -079 ; 0235D
DW -078 ; 0236D
DW -076 ; 0237D
DW -075 ; 0238D
DW -074 ; 0239D
DW -072 ; 0240D
DW -071 ; 0241D
DW -070 ; 0242D
DW -068 ; 0243D
DW -067 ; 0244D
DW -065 ; 0245D
DW -064 ; 0246D
DW -063 ; 0247D
DW -061 ; 0248D
DW -060 ; 0249D
DW -059 ; 0250D
DW -057 ; 0251D
DW -056 ; 0252D
DW -055 ; 0253D
DW -053 ; 0254D
DW -052 ; 0255D
DW -050 ; 0256D
DW -049 ; 0257D
DW -048 ; 0258D
DW -046 ; 0259D
DW -045 ; 0260D
DW -044 ; 0261D
DW -042 ; 0262D
DW -041 ; 0263D
DW -040 ; 0264D
DW -038 ; 0265D
DW -037 ; 0266D
DW -035 ; 0267D
DW -034 ; 0268D
DW -033 ; 0269D
DW -031 ; 0270D
DW -030 ; 0271D
DW -029 ; 0272D
DW -027 ; 0273D
DW -026 ; 0274D
DW -024 ; 0275D
DW -023 ; 0276D
DW -022 ; 0277D
DW -020 ; 0278D
DW -019 ; 0279D
DW -018 ; 0280D
DW -016 ; 0281D
DW -015 ; 0282D
DW -014 ; 0283D
DW -012 ; 0284D
DW -011 ; 0285D
DW -009 ; 0286D
DW -008 ; 0287D
DW -007 ; 0288D
DW -005 ; 0289D
DW -004 ; 0290D
DW -003 ; 0291D
DW -001 ; 0292D
DW 0000 ; 0293D
DW 0001 ; 0294D
DW 0003 ; 0295D
DW 0004 ; 0296D
DW 0006 ; 0297D
DW 0007 ; 0298D
DW 0008 ; 0299D
DW 0010 ; 0300D
DW 0011 ; 0301D
DW 0012 ; 0302D
DW 0014 ; 0303D
DW 0015 ; 0304D
DW 0016 ; 0305D
DW 0018 ; 0306D
DW 0019 ; 0307D
DW 0021 ; 0308D
DW 0022 ; 0309D
DW 0023 ; 0310D
DW 0025 ; 0311D
DW 0026 ; 0312D
DW 0027 ; 0313D
DW 0029 ; 0314D
DW 0030 ; 0315D
DW 0032 ; 0316D
DW 0033 ; 0317D
DW 0034 ; 0318D
DW 0036 ; 0319D
DW 0037 ; 0320D
DW 0038 ; 0321D
DW 0040 ; 0322D
DW 0041 ; 0323D
DW 0042 ; 0324D
DW 0044 ; 0325D
DW 0045 ; 0326D
DW 0047 ; 0327D
DW 0048 ; 0328D
DW 0049 ; 0329D
DW 0051 ; 0330D
DW 0052 ; 0331D
DW 0053 ; 0332D
DW 0055 ; 0333D
DW 0056 ; 0334D
DW 0057 ; 0335D
DW 0059 ; 0336D
DW 0060 ; 0337D
DW 0062 ; 0338D
DW 0063 ; 0339D
DW 0064 ; 0340D
DW 0066 ; 0341D
DW 0067 ; 0342D
DW 0068 ; 0343D
DW 0070 ; 0344D
DW 0071 ; 0345D
DW 0072 ; 0346D
DW 0074 ; 0347D
DW 0075 ; 0348D
DW 0077 ; 0349D
DW 0078 ; 0350D
DW 0079 ; 0351D
DW 0081 ; 0352D
DW 0082 ; 0353D
DW 0083 ; 0354D
DW 0085 ; 0355D
DW 0086 ; 0356D
DW 0087 ; 0357D
DW 0089 ; 0358D
DW 0090 ; 0359D
DW 0092 ; 0360D
DW 0093 ; 0361D
DW 0094 ; 0362D
DW 0096 ; 0363D
DW 0097 ; 0364D
DW 0098 ; 0365D
DW 0100 ; 0366D
DW 0101 ; 0367D
DW 0103 ; 0368D
DW 0104 ; 0369D
DW 0105 ; 0370D
DW 0107 ; 0371D
DW 0108 ; 0372D
DW 0109 ; 0373D
DW 0111 ; 0374D
DW 0112 ; 0375D
DW 0113 ; 0376D
DW 0115 ; 0377D
DW 0116 ; 0378D
DW 0118 ; 0379D
DW 0119 ; 0380D
DW 0120 ; 0381D
DW 0122 ; 0382D
DW 0123 ; 0383D
DW 0124 ; 0384D
DW 0126 ; 0385D
DW 0127 ; 0386D
DW 0128 ; 0387D
DW 0130 ; 0388D
DW 0131 ; 0389D
DW 0133 ; 0390D
DW 0134 ; 0391D
DW 0135 ; 0392D
DW 0137 ; 0393D
DW 0138 ; 0394D
DW 0139 ; 0395D
DW 0141 ; 0396D
DW 0142 ; 0397D
DW 0143 ; 0398D
DW 0145 ; 0399D
DW 0146 ; 0400D
DW 0148 ; 0401D
DW 0149 ; 0402D
DW 0150 ; 0403D
DW 0152 ; 0404D
DW 0153 ; 0405D
DW 0154 ; 0406D
DW 0156 ; 0407D
DW 0157 ; 0408D
DW 0159 ; 0409D
DW 0160 ; 0410D
DW 0161 ; 0411D
DW 0163 ; 0412D
DW 0164 ; 0413D
DW 0165 ; 0414D
DW 0167 ; 0415D
DW 0168 ; 0416D
DW 0169 ; 0417D
DW 0171 ; 0418D
DW 0172 ; 0419D
DW 0174 ; 0420D
DW 0175 ; 0421D
DW 0176 ; 0422D
DW 0178 ; 0423D
DW 0179 ; 0424D
DW 0180 ; 0425D
DW 0182 ; 0426D
DW 0183 ; 0427D
DW 0184 ; 0428D
DW 0186 ; 0429D
DW 0187 ; 0430D
DW 0189 ; 0431D
DW 0190 ; 0432D
DW 0191 ; 0433D
DW 0193 ; 0434D
DW 0194 ; 0435D
DW 0195 ; 0436D
DW 0197 ; 0437D
DW 0198 ; 0438D
DW 0199 ; 0439D
DW 0201 ; 0440D
DW 0202 ; 0441D
DW 0204 ; 0442D
DW 0205 ; 0443D
DW 0206 ; 0444D
DW 0208 ; 0445D
DW 0209 ; 0446D
DW 0210 ; 0447D
DW 0212 ; 0448D
DW 0213 ; 0449D
DW 0214 ; 0450D
DW 0216 ; 0451D
DW 0217 ; 0452D
DW 0219 ; 0453D
DW 0220 ; 0454D
DW 0221 ; 0455D
DW 0223 ; 0456D
DW 0224 ; 0457D
DW 0225 ; 0458D
DW 0227 ; 0459D
DW 0228 ; 0460D
DW 0230 ; 0461D
DW 0231 ; 0462D
DW 0232 ; 0463D
DW 0234 ; 0464D
DW 0235 ; 0465D
DW 0236 ; 0466D
DW 0238 ; 0467D
DW 0239 ; 0468D
DW 0240 ; 0469D
DW 0242 ; 0470D
DW 0243 ; 0471D
DW 0245 ; 0472D
DW 0246 ; 0473D
DW 0247 ; 0474D
DW 0249 ; 0475D
DW 0250 ; 0476D
DW 0251 ; 0477D
DW 0253 ; 0478D
DW 0254 ; 0479D
DW 0255 ; 0480D
DW 0257 ; 0481D
DW 0258 ; 0482D
DW 0260 ; 0483D
DW 0261 ; 0484D
DW 0262 ; 0485D
DW 0264 ; 0486D
DW 0265 ; 0487D
DW 0266 ; 0488D
DW 0268 ; 0489D
DW 0269 ; 0490D
DW 0270 ; 0491D
DW 0272 ; 0492D
DW 0273 ; 0493D
DW 0275 ; 0494D
DW 0276 ; 0495D
DW 0277 ; 0496D
DW 0279 ; 0497D
DW 0280 ; 0498D
DW 0281 ; 0499D
DW 0283 ; 0500D
DW 0284 ; 0501D
DW 0285 ; 0502D
DW 0287 ; 0503D
DW 0288 ; 0504D
DW 0290 ; 0505D
DW 0291 ; 0506D
DW 0292 ; 0507D
DW 0294 ; 0508D
DW 0295 ; 0509D
DW 0296 ; 0510D
DW 0298 ; 0511D
DW 0299 ; 0512D
DW 0301 ; 0513D
DW 0302 ; 0514D
DW 0303 ; 0515D
DW 0305 ; 0516D
DW 0306 ; 0517D
DW 0307 ; 0518D
DW 0309 ; 0519D
DW 0310 ; 0520D
DW 0311 ; 0521D
DW 0313 ; 0522D
DW 0314 ; 0523D
DW 0316 ; 0524D
DW 0317 ; 0525D
DW 0318 ; 0526D
DW 0320 ; 0527D
DW 0321 ; 0528D
DW 0322 ; 0529D
DW 0324 ; 0530D
DW 0325 ; 0531D
DW 0326 ; 0532D
DW 0328 ; 0533D
DW 0329 ; 0534D
DW 0331 ; 0535D
DW 0332 ; 0536D
DW 0333 ; 0537D
DW 0335 ; 0538D
DW 0336 ; 0539D
DW 0337 ; 0540D
DW 0339 ; 0541D
DW 0340 ; 0542D
DW 0341 ; 0543D
DW 0343 ; 0544D
DW 0344 ; 0545D
DW 0346 ; 0546D
DW 0347 ; 0547D
DW 0348 ; 0548D
DW 0350 ; 0549D
DW 0351 ; 0550D
DW 0352 ; 0551D
DW 0354 ; 0552D
DW 0355 ; 0553D
DW 0357 ; 0554D
DW 0358 ; 0555D
DW 0359 ; 0556D
DW 0361 ; 0557D
DW 0362 ; 0558D
DW 0363 ; 0559D
DW 0365 ; 0560D
DW 0366 ; 0561D
DW 0367 ; 0562D
DW 0369 ; 0563D
DW 0370 ; 0564D
DW 0372 ; 0565D
DW 0373 ; 0566D
DW 0374 ; 0567D
DW 0376 ; 0568D
DW 0377 ; 0569D
DW 0378 ; 0570D
DW 0380 ; 0571D
DW 0381 ; 0572D
DW 0382 ; 0573D
DW 0384 ; 0574D
DW 0385 ; 0575D
DW 0387 ; 0576D
DW 0388 ; 0577D
DW 0389 ; 0578D
DW 0391 ; 0579D
DW 0392 ; 0580D
DW 0393 ; 0581D
DW 0395 ; 0582D
DW 0396 ; 0583D
DW 0397 ; 0584D
DW 0399 ; 0585D
DW 0400 ; 0586D
DW 0402 ; 0587D
DW 0403 ; 0588D
DW 0404 ; 0589D
DW 0406 ; 0590D
DW 0407 ; 0591D
DW 0408 ; 0592D
DW 0410 ; 0593D
DW 0411 ; 0594D
DW 0412 ; 0595D
DW 0414 ; 0596D
DW 0415 ; 0597D
DW 0417 ; 0598D
DW 0418 ; 0599D
DW 0419 ; 0600D
DW 0421 ; 0601D
DW 0422 ; 0602D
DW 0423 ; 0603D
DW 0425 ; 0604D
DW 0426 ; 0605D
DW 0428 ; 0606D
DW 0429 ; 0607D
DW 0430 ; 0608D
DW 0432 ; 0609D
DW 0433 ; 0610D
DW 0434 ; 0611D
DW 0436 ; 0612D
DW 0437 ; 0613D
DW 0438 ; 0614D
DW 0440 ; 0615D
DW 0441 ; 0616D
DW 0443 ; 0617D
DW 0444 ; 0618D
DW 0445 ; 0619D
DW 0447 ; 0620D
DW 0448 ; 0621D
DW 0449 ; 0622D
DW 0451 ; 0623D
DW 0452 ; 0624D
DW 0453 ; 0625D
DW 0455 ; 0626D
DW 0456 ; 0627D
DW 0458 ; 0628D
DW 0459 ; 0629D
DW 0460 ; 0630D
DW 0462 ; 0631D
DW 0463 ; 0632D
DW 0464 ; 0633D
DW 0466 ; 0634D
DW 0467 ; 0635D
DW 0468 ; 0636D
DW 0470 ; 0637D
DW 0471 ; 0638D
DW 0473 ; 0639D
DW 0474 ; 0640D
DW 0475 ; 0641D
DW 0477 ; 0642D
DW 0478 ; 0643D
DW 0479 ; 0644D
DW 0481 ; 0645D
DW 0482 ; 0646D
DW 0483 ; 0647D
DW 0485 ; 0648D
DW 0486 ; 0649D
DW 0488 ; 0650D
DW 0489 ; 0651D
DW 0490 ; 0652D
DW 0492 ; 0653D
DW 0493 ; 0654D
DW 0494 ; 0655D
DW 0496 ; 0656D
DW 0497 ; 0657D
DW 0499 ; 0658D
DW 0500 ; 0659D
DW 0501 ; 0660D
DW 0503 ; 0661D
DW 0504 ; 0662D
DW 0505 ; 0663D
DW 0507 ; 0664D
DW 0508 ; 0665D
DW 0509 ; 0666D
DW 0511 ; 0667D
DW 0512 ; 0668D
DW 0514 ; 0669D
DW 0515 ; 0670D
DW 0516 ; 0671D
DW 0518 ; 0672D
DW 0519 ; 0673D
DW 0520 ; 0674D
DW 0522 ; 0675D
DW 0523 ; 0676D
DW 0524 ; 0677D
DW 0526 ; 0678D
DW 0527 ; 0679D
DW 0529 ; 0680D
DW 0530 ; 0681D
DW 0531 ; 0682D
DW 0533 ; 0683D
DW 0534 ; 0684D
DW 0535 ; 0685D
DW 0537 ; 0686D
DW 0538 ; 0687D
DW 0539 ; 0688D
DW 0541 ; 0689D
DW 0542 ; 0690D
DW 0544 ; 0691D
DW 0545 ; 0692D
DW 0546 ; 0693D
DW 0548 ; 0694D
DW 0549 ; 0695D
DW 0550 ; 0696D
DW 0552 ; 0697D
DW 0553 ; 0698D
DW 0555 ; 0699D
DW 0556 ; 0700D
DW 0557 ; 0701D
DW 0559 ; 0702D
DW 0560 ; 0703D
DW 0561 ; 0704D
DW 0563 ; 0705D
DW 0564 ; 0706D
DW 0565 ; 0707D
DW 0567 ; 0708D
DW 0568 ; 0709D
DW 0570 ; 0710D
DW 0571 ; 0711D
DW 0572 ; 0712D
DW 0574 ; 0713D
DW 0575 ; 0714D
DW 0576 ; 0715D
DW 0578 ; 0716D
DW 0579 ; 0717D
DW 0580 ; 0718D
DW 0582 ; 0719D
DW 0583 ; 0720D
DW 0585 ; 0721D
DW 0586 ; 0722D
DW 0587 ; 0723D
DW 0589 ; 0724D
DW 0590 ; 0725D
DW 0591 ; 0726D
DW 0593 ; 0727D
DW 0594 ; 0728D
DW 0595 ; 0729D
DW 0597 ; 0730D
DW 0598 ; 0731D
DW 0600 ; 0732D
DW 0601 ; 0733D
DW 0602 ; 0734D
DW 0604 ; 0735D
DW 0605 ; 0736D
DW 0606 ; 0737D
DW 0608 ; 0738D
DW 0609 ; 0739D
DW 0610 ; 0740D
DW 0612 ; 0741D
DW 0613 ; 0742D
DW 0615 ; 0743D
DW 0616 ; 0744D
DW 0617 ; 0745D
DW 0619 ; 0746D
DW 0620 ; 0747D
DW 0621 ; 0748D
DW 0623 ; 0749D
DW 0624 ; 0750D
DW 0626 ; 0751D
DW 0627 ; 0752D
DW 0628 ; 0753D
DW 0630 ; 0754D
DW 0631 ; 0755D
DW 0632 ; 0756D
DW 0634 ; 0757D
DW 0635 ; 0758D
DW 0636 ; 0759D
DW 0638 ; 0760D
DW 0639 ; 0761D
DW 0641 ; 0762D
DW 0642 ; 0763D
DW 0643 ; 0764D
DW 0645 ; 0765D
DW 0646 ; 0766D
DW 0647 ; 0767D
DW 0649 ; 0768D
DW 0650 ; 0769D
DW 0651 ; 0770D
DW 0653 ; 0771D
DW 0654 ; 0772D
DW 0656 ; 0773D
DW 0657 ; 0774D
DW 0658 ; 0775D
DW 0660 ; 0776D
DW 0661 ; 0777D
DW 0662 ; 0778D
DW 0664 ; 0779D
DW 0665 ; 0780D
DW 0666 ; 0781D
DW 0668 ; 0782D
DW 0669 ; 0783D
DW 0671 ; 0784D
DW 0672 ; 0785D
DW 0673 ; 0786D
DW 0675 ; 0787D
DW 0676 ; 0788D
DW 0677 ; 0789D
DW 0679 ; 0790D
DW 0680 ; 0791D
DW 0681 ; 0792D
DW 0683 ; 0793D
DW 0684 ; 0794D
DW 0686 ; 0795D
DW 0687 ; 0796D
DW 0688 ; 0797D
DW 0690 ; 0798D
DW 0691 ; 0799D
DW 0692 ; 0800D
DW 0694 ; 0801D
DW 0695 ; 0802D
DW 0697 ; 0803D
DW 0698 ; 0804D
DW 0699 ; 0805D
DW 0701 ; 0806D
DW 0702 ; 0807D
DW 0703 ; 0808D
DW 0705 ; 0809D
DW 0706 ; 0810D
DW 0707 ; 0811D
DW 0709 ; 0812D
DW 0710 ; 0813D
DW 0712 ; 0814D
DW 0713 ; 0815D
DW 0714 ; 0816D
DW 0716 ; 0817D
DW 0717 ; 0818D
DW 0718 ; 0819D
DW 0720 ; 0820D
DW 0721 ; 0821D
DW 0722 ; 0822D
DW 0724 ; 0823D
DW 0725 ; 0824D
DW 0727 ; 0825D
DW 0728 ; 0826D
DW 0729 ; 0827D
DW 0731 ; 0828D
DW 0732 ; 0829D
DW 0733 ; 0830D
DW 0735 ; 0831D
DW 0736 ; 0832D
DW 0737 ; 0833D
DW 0739 ; 0834D
DW 0740 ; 0835D
DW 0742 ; 0836D
DW 0743 ; 0837D
DW 0744 ; 0838D
DW 0746 ; 0839D
DW 0747 ; 0840D
DW 0748 ; 0841D
DW 0750 ; 0842D
DW 0751 ; 0843D
DW 0753 ; 0844D
DW 0754 ; 0845D
DW 0755 ; 0846D
DW 0757 ; 0847D
DW 0758 ; 0848D
DW 0759 ; 0849D
DW 0761 ; 0850D
DW 0762 ; 0851D
DW 0763 ; 0852D
DW 0765 ; 0853D
DW 0766 ; 0854D
DW 0768 ; 0855D
DW 0769 ; 0856D
DW 0770 ; 0857D
DW 0772 ; 0858D
DW 0773 ; 0859D
DW 0774 ; 0860D
DW 0776 ; 0861D
DW 0777 ; 0862D
DW 0778 ; 0863D
DW 0780 ; 0864D
DW 0781 ; 0865D
DW 0783 ; 0866D
DW 0784 ; 0867D
DW 0785 ; 0868D
DW 0787 ; 0869D
DW 0788 ; 0870D
DW 0789 ; 0871D
DW 0791 ; 0872D
DW 0792 ; 0873D
DW 0793 ; 0874D
DW 0795 ; 0875D
DW 0796 ; 0876D
DW 0798 ; 0877D
DW 0799 ; 0878D
DW 0800 ; 0879D
DW 0802 ; 0880D
DW 0803 ; 0881D
DW 0804 ; 0882D
DW 0806 ; 0883D
DW 0807 ; 0884D
DW 0808 ; 0885D
DW 0810 ; 0886D
DW 0811 ; 0887D
DW 0813 ; 0888D
DW 0814 ; 0889D
DW 0815 ; 0890D
DW 0817 ; 0891D
DW 0818 ; 0892D
DW 0819 ; 0893D
DW 0821 ; 0894D
DW 0822 ; 0895D
DW 0824 ; 0896D
DW 0825 ; 0897D
DW 0826 ; 0898D
DW 0828 ; 0899D
DW 0829 ; 0900D
DW 0830 ; 0901D
DW 0832 ; 0902D
DW 0833 ; 0903D
DW 0834 ; 0904D
DW 0836 ; 0905D
DW 0837 ; 0906D
DW 0839 ; 0907D
DW 0840 ; 0908D
DW 0841 ; 0909D
DW 0843 ; 0910D
DW 0844 ; 0911D
DW 0845 ; 0912D
DW 0847 ; 0913D
DW 0848 ; 0914D
DW 0849 ; 0915D
DW 0851 ; 0916D
DW 0852 ; 0917D
DW 0854 ; 0918D
DW 0855 ; 0919D
DW 0856 ; 0920D
DW 0858 ; 0921D
DW 0859 ; 0922D
DW 0860 ; 0923D
DW 0862 ; 0924D
DW 0863 ; 0925D
DW 0864 ; 0926D
DW 0866 ; 0927D
DW 0867 ; 0928D
DW 0869 ; 0929D
DW 0870 ; 0930D
DW 0871 ; 0931D
DW 0873 ; 0932D
DW 0874 ; 0933D
DW 0875 ; 0934D
DW 0877 ; 0935D
DW 0878 ; 0936D
DW 0880 ; 0937D
DW 0881 ; 0938D
DW 0882 ; 0939D
DW 0884 ; 0940D
DW 0885 ; 0941D
DW 0886 ; 0942D
DW 0888 ; 0943D
DW 0889 ; 0944D
DW 0890 ; 0945D
DW 0892 ; 0946D
DW 0893 ; 0947D
DW 0895 ; 0948D
DW 0896 ; 0949D
DW 0897 ; 0950D
DW 0899 ; 0951D
DW 0900 ; 0952D
DW 0901 ; 0953D
DW 0903 ; 0954D
DW 0904 ; 0955D
DW 0905 ; 0956D
DW 0907 ; 0957D
DW 0908 ; 0958D
DW 0910 ; 0959D
DW 0911 ; 0960D
DW 0912 ; 0961D
DW 0914 ; 0962D
DW 0915 ; 0963D
DW 0916 ; 0964D
DW 0918 ; 0965D
DW 0919 ; 0966D
DW 0920 ; 0967D
DW 0922 ; 0968D
DW 0923 ; 0969D
DW 0925 ; 0970D
DW 0926 ; 0971D
DW 0927 ; 0972D
DW 0929 ; 0973D
DW 0930 ; 0974D
DW 0931 ; 0975D
DW 0933 ; 0976D
DW 0934 ; 0977D
DW 0935 ; 0978D
DW 0937 ; 0979D
DW 0938 ; 0980D
DW 0940 ; 0981D
DW 0941 ; 0982D
DW 0942 ; 0983D
DW 0944 ; 0984D
DW 0945 ; 0985D
DW 0946 ; 0986D
DW 0948 ; 0987D
DW 0949 ; 0988D
DW 0951 ; 0989D
DW 0952 ; 0990D
DW 0953 ; 0991D
DW 0955 ; 0992D
DW 0956 ; 0993D
DW 0957 ; 0994D
DW 0959 ; 0995D
DW 0960 ; 0996D
DW 0961 ; 0997D
DW 0963 ; 0998D
DW 0964 ; 0999D
DW 0966 ; 1000D
DW 0967 ; 1001D
DW 0968 ; 1002D
DW 0970 ; 1003D
DW 0971 ; 1004D
DW 0972 ; 1005D
DW 0974 ; 1006D
DW 0975 ; 1007D
DW 0976 ; 1008D
DW 0978 ; 1009D
DW 0979 ; 1010D
DW 0981 ; 1011D
DW 0982 ; 1012D
DW 0983 ; 1013D
DW 0985 ; 1014D
DW 0986 ; 1015D
DW 0987 ; 1016D
DW 0989 ; 1017D
DW 0990 ; 1018D
DW 0991 ; 1019D
DW 0993 ; 1020D
DW 0994 ; 1021D
DW 0996 ; 1022D
DW 0997 ; 1023D

Display:
	mov dptr, #myLUT
	; Display Digit 0
    mov A, bcd+0
    anl a, #0fh
    movc A, @A+dptr
    mov HEX0, A
	; Display Digit 1
    mov A, bcd+0
    swap a
    anl a, #0fh
    movc A, @A+dptr
    mov HEX1, A
	; Display Digit 2
    mov A, bcd+1
    anl a, #0fh
    movc A, @A+dptr
    mov HEX2, A
ret

positive:
mov HEX3, #7FH
ret

signcheck:
mov a, x+1
jb acc.7, negative
jnb acc.7, positive              ; Turns off LEDR1 if value is positive
ret

negative:
mov HEX3, #03FH
clr a
subb a, x+0
mov x+0, a
clr a
subb a, x+1
mov x+1, a
ret

lookup:
mov SP, #7FH 	; Initializes the Stack Pointer, set high
clr a        	; Sets a to zero
mov LEDRA, a    ; Turn off LEDs
mov LEDRB, a
mov LEDRC, a
mov LEDG, a
mov bcd+0, a
mov bcd+1, a
mov bcd+2, a

;takes input and stores in R6 and R7
mov R6, SWA
mov a, SWB
anl a, #00000011B
mov R7, a

;converts the input to the value of data pointer
mov dptr, #mytable
mov a, R6
mov b, #2
mul ab
add a, dpl
mov dpl, a

mov R5, b
mov a, R7
rlc a
add a, R5
addc a, dph
mov dph, a

clr a
movc a, @a+dptr
mov x+1, a ; High part from the lookup table
inc dptr
clr a
movc a, @a+dptr
mov x+0, a 

lcall signcheck
lcall hex2bcd
lcall Display
ljmp lookup

end
