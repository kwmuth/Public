...
{
    /* Variables */
	double readings         [NUMBER_OF_READINGS+4]; /* Copy the data file to this array */
	 ...
	 */
	 ...
	 for( i = 0; i < NUMBER_OF_READINGS+4 ; i++ ){
		 readings[i] = 0;
	 }
	 ...
	 */
	 for (i = 0; i < (NUMBER_OF_READINGS+4)/2; i++ ){
...
void process_file( double array_to_populate[], FILE * pointer_to_data_file )
{
	...
	while( fgets( line_buffer, LINESIZE, pointer_to_data_file ) != NULL ) { 
		...
		values_per_line = sscanf_s( line_buffer, "%d %d %d %d %d", &extracted_values[0], &extracted_values[1], 
			&extracted_values[2], &extracted_values[3], &extracted_values[4] );
		...
		/* Copies the extracted integers to our data array.  Use a for loop for each
		 ...
		 {
			 array_to_populate[x] = extracted_values[i];
			 x++;
		 }
