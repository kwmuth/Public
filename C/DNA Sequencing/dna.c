 ...
 *                  sequence file is a txt file (two samples
 ...
 * a valid menu choice.
 *
 * Uses fgets and sscanf_s ensures that we only accept correct
 ...
 *    the perfect matches using a message of this format:
 ...
 * c) Once a best match is found, the scores for all the candidates formatted like this:
...
void analyze_segments( char * sample_segment, char ** candidate_segments, int number_of_candidates )
{
	...
	{
		candidate_length = strlen( *(candidate_segments + x) );

		if( candidate_length == sample_length )
		{
			while( i < sample_length )
			{
				if( *(sample_segment + i) == *(*(candidate_segments + x) + i ) )
					i++;
				else
					i = sample_length + 1;
			}
 ...
 *    b) if the 2 codons are different but specify the same amino acid, add 5 to the score
 *    c) if the two codons are different and do not specify the same amino acid, then
 ...
 *    and check the score of this new alignment.  Store the higher score.  Keep shifting
 *    the sample deeper into the candidate by one codon at a time, checking the score, until it
 *    no longer fits. Once the sample extends beyond the length of the candidate we stop.
 * PARAM:     pointer to the sample segment
...
int calculate_score( char * sample_segment, char * candidate_segment)
{
	...
	int a							= 0;
	int nucleotide					= 0;
	int address_sample				= 0;
	int address_candidate			= 0;
	int count						= 0;
	int offset						= 0;
	int temp_score					= 0;
	int score						= 0;
	...
	int segment_difference; 
	char* array_sample;
	char* array_candidate;
	char* address_sample_ptr;
	char* address_candidate_ptr;
	
	segment_difference = candidate_length_in_codons - sample_length_in_codons;

	array_sample = (char* )malloc( 3*sizeof( char ));
	array_candidate = (char* )malloc( 3*sizeof( char )); 
	
	for( iterations = 0; iterations <= segment_difference; iterations++ )
	{
		x = 0;
		temp_score = 0;

		while( x < (sample_length/3)*3 ) 
		{
			a = 0;
			count = 0;
			...
			for( nucleotide = 0; nucleotide < CODON_LENGTH; nucleotide++ )
			{
				...
				if( sample_segment[x] == candidate_segment[y] )
					count++;

				*(array_sample + a) = sample_segment[x];
				*(array_candidate + a) = candidate_segment[y];
				x++; 
				y++;
				a++;
			}

			address_sample = get_codon_index( array_sample );
			address_candidate = get_codon_index( array_candidate );

			address_sample_ptr = &codon_names[address_sample][0];
			address_candidate_ptr = &codon_names[address_candidate][0];

			/* all 3 nucleotides in the codon were a match, add 10 to score */
			if( count == 3 )		
				temp_score += 10;

			/* if same amino acid, add 5 to score */
			else if( strncmp( address_sample_ptr, address_candidate_ptr, CODON_NAME_LENGTH ) == 0 )
			{
					temp_score+=5;
			}
			...
			else
			{
				...
				for( a = 0; a < CODON_LENGTH; a++ )
				{
					value = is_base_pair( *(array_sample + a), *(array_candidate + a) );
